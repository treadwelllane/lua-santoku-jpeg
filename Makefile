...
LOCAL_CFLAGS ?= $(if $(LUA_INCDIR), -I$(LUA_INCDIR)) -I $(TEST_JPEG_DIR)
LOCAL_LDFLAGS ?= $(if $(LUA_LIBDIR), -L$(LUA_LIBDIR)) -L $(TEST_JPEG_DIR)/.libs -ljpeg
...
TEST_CFLAGS ?= -gsource-map -I $(TEST_LUA_INC_DIR) -I $(TEST_JPEG_DIR) --bind -sALLOW_MEMORY_GROWTH $(SANITIZER_FLAGS)
